 /* VC§«•≥•Û•—•§•ÅEπ§ÅE»§≠§œ windows.h §Œ•≥•·•Û•»§Ú≥∞§π */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <windows.h>
...
/* ¿ÅE¿…ÅE*/
/* •Ÿ•Ø•»•ÅE»§Ω§Œ¬Á§≠§µ§ŒπΩ¬§¬Œ */
typedef struct{
	GLfloat vector[3]; /* •Ÿ•Ø•»•ÅEector[XYZ] */
	GLfloat norm;  /* •Ÿ•Ø•»•ÅEŒ¬Á§≠§µ */
} Vector;

/*  ™¬Œ§Œ¿ﬂƒÍ§ŒπΩ¬§¬Œ¿ÅE¿ */
typedef struct{
	GLfloat diffuse_rgba[4];     /*  ™¬Œ§Œ≥»ª∂∏ÅE*/
	GLfloat specular_rgba[4];    /*  ™¬Œ§Œ∂¿ÃÃ»øºÕ∏ÅE*/
	GLfloat shininess_rate;      /* ∂¿ÃÃ»øºÕªÿøÅE*/
} Material;

/* º´µ°§ŒπΩ¬§¬Œ¿ÅE¿ */
typedef struct{
	Vector position;       /* ∞Ã√÷•Ÿ•Ø•»•ÅE*/
	Vector speed;          /* ¬Æ≈Ÿ•Ÿ•Ø•»•ÅE*/
	Vector direction;      /* ∏˛§≠ */
	GLfloat power;          /* Ω–Œœ */
	Vector mobility;       /* µ°∆∞¿≠ */
	GLint life;            /* ªƒµ° */
	GLint balls;           /* ª˝§¡∂Ã */
	GLfloat hitwidth,hitwidth0[2],hitheight,hitdepth;     /* ≈ˆ§ø§ÅEΩƒÍ§Œ¬Á§≠§µ */
    Material material;     /*  ™¬Œ§»§∑§∆§Œ¿ﬂƒÅE*/
	int score;             /* ∆¿≈¿ */
	int highscore;         /* •œ•§•π•≥•¢ */
	GLint deadcount;
	GLint newrecoad;
...
typedef struct{
	Vector position;       /* ∞Ã√÷•Ÿ•Ø•»•ÅE*/
	Vector speed;          /* ¬Æ≈Ÿ•Ÿ•Ø•»•ÅE*/
	Vector addspeed;       /* ≤√¬Æ≈Ÿ•Ÿ•Ø•»•ÅE*/
    Material material;     /*  ™¬Œ§»§∑§∆§Œ¿ﬂƒÅE*/	
	GLint tamaflag;        /* √∆§ŒÕ≠Ãµ */
	GLint hitcount;        /* œ¢¬≥•“•√•»§Œ•´•¶•Û•ø°º */
	GLint point;           /* ∆˛§ÅE¿≈¿ */
	GLint hitflag[10][10];
	GLint time;           /* ¬∏∫ﬂª˛¥÷ */
} Tama;
...
typedef struct{
	...
	GLint gravity_flag;    /* -1:¿ÕŒœ 0:Ãµ§∑ 1:∞˙Œœ */
	GLfloat blockroll[10][10];           /* •÷•˙¡√•Ø§Œ≤Û≈æ≥—≈Ÿ */
	GLfloat blockrollspeed[20];           /* •÷•˙¡√•Ø§Œ≤Û≈æ≥—≈Ÿ */
	GLint blockpoint[20];         /* •÷•˙¡√•Ø§Œ∆¿≈¿ */
	GLint enemy;
	char filename[16][16];   /* •’•°•§•ÅEæ */
	char filenamedata[16];   /* •π•∆°º•∏ÀË§Œ•’•°•§•ÅEæ§Œ•«°º•ø */
	GLint now;    /* ∏Ω∫ﬂ§Œ•π•∆°º•∏ */
	GLint last; /* ∫«Ω™•π•∆°º•∏ */
	GLint demotime; /* ≈–æÅEø•Ø•ÅE¢•∑°º•Û§Œªƒ§ÅE˛¥÷ */
	GLint display;
	GLfloat vmax[2];  /* •‹°º•ÅEŒ∫«π‚¬Æ≈Ÿ */
	GLint debugflag;
	GLfloat demoview[9];
	GLfloat gameview[9];
} Stage;
...
{
	GLfloat v[4] = {1.0,1.0,1.0,1.0}; /* RGBA §Œ§«°º§ø */
	glLightfv(GL_LIGHT0,GL_DIFFUSE,v);    /* «Ú§§∏˜∏ª */
	/* glLightModelf(GL_LIGHT_MODEL_LOCAL_VIEWER,GL_TRUE); */ /* ∂¿ÃÃ»øºÕΩËÕ˝§Ú•ÅE¢•ÅEÀ */
    glEnable(GL_LIGHTING); glEnable(GL_LIGHT0);  /* •È•§•»§Œ¿ﬂƒÍ§ÚÕ≠∏˙§À */
	glEnable(GL_NORMALIZE);   /* À°¿˛∑◊ªª§Ú¿µµ¨≤Ω */
	glEnable(GL_DEPTH_TEST);  /* •«•◊•π•–•√•’•°§Úª»Õ— */
}
...
{
	static GLfloat v[4]={1.0,-1.0,2.0,0.0}; /* ∏˜∏ª§Œ∏˛§≠ */
	auxInitPosition(0,0,800,800);
	...
	glLightfv(GL_LIGHT0,GL_POSITION,v);      /*  øπ‘∏˜¿˛ */
	glClearColor(0.0,0.0,0.0,0.0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
