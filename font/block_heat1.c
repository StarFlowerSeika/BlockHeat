/* VC梡偺偨傔windows.h傪愭偵include偡傞昁梫偑偁傞 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <windows.h>
...
/* 儀僋僩儖宆 */
typedef struct{
	GLfloat vector[3]; /* 儀僋僩儖[XYZ] */
	GLfloat norm;      /* 儀僋僩儖偺戝偒偝 */
} Vector;

/* 儅僥儕傾儖乮嵽幙乯宆 */
typedef struct {
	GLfloat diffuse_rgba[4];     /* 奼嶶斀幩怓 */
	GLfloat specular_rgba[4];    /* 嬀柺斀幩怓 */
	GLfloat shininess_rate;      /* 婸偒偺搙崌偄 */
} Material;


/* 帺婡偺宆 */
typedef struct {
	Vector position;       /* 埵抲儀僋僩儖 */
	Vector speed;          /* 懍搙儀僋僩儖 */
	Vector direction;      /* 岦偒儀僋僩儖 */
	GLfloat power;         /* 僷儚乕 */
	Vector mobility;       /* 婡摦椡 */
	GLint life;            /* 巆婡 */
	GLint balls;           /* 儃乕儖悢 */
	GLfloat hitwidth, hitwidth0[2], hitheight, hitdepth; /* 摉偨傝敾掕僒僀僘 */
	Material material;     /* 嵽幙 */
	int score;             /* 僗僐傾 */
	int highscore;         /* 僴僀僗僐傾 */
	GLint deadcount;       /* 巰朣夞悢 */
	GLint newrecoad;       /* 怴婰榐僼儔僌 */
	GLint gamemode;        /* 僎乕儉儌乕僪 */
} Ship;
...
typedef struct {
	Vector position;       /* 埵抲儀僋僩儖 */
	Vector speed;          /* 懍搙儀僋僩儖 */
	Vector addspeed;       /* 壛懍搙儀僋僩儖 */
	Material material;     /* 嵽幙 */
	GLint tamaflag;        /* 抏偺桳岠僼儔僌 */
	GLint hitcount;        /* 僸僢僩悢 */
	GLint point;           /* 摼揰 */
	GLint hitflag[10][10];
	GLint time;            /* 宱夁帪娫 */
...
typedef struct {
	...
	GLint gravity_flag;    /* -1:壓岦偒 0:側偟 1:忋岦偒 */
	GLfloat blockroll[10][10];           /* 僽儘僢僋夞揮 */
	GLfloat blockrollspeed[20];          /* 僽儘僢僋夞揮懍搙 */
	GLint blockpoint[20];                /* 僽儘僢僋摼揰 */
	GLint enemy;
	char filename[16][16];   /* 僼傽僀儖柤儕僗僩 */
	char filenamedata[16];   /* 僗僥乕僕僼傽僀儖柤 */
	GLint now;               /* 尰嵼偺僗僥乕僕斣崋 */
	GLint last;              /* 嵟廔僗僥乕僕斣崋 */
	GLint demotime;          /* 僨儌帪娫 */
	GLint display;
...
{
	GLfloat v[4] = { 1.0,1.0,1.0,1.0 }; /* RGBA岝尮怓 */
	glLightfv(GL_LIGHT0, GL_DIFFUSE, v);    /* 奼嶶岝愝掕 */
	/* glLightModelf(GL_LIGHT_MODEL_LOCAL_VIEWER,GL_TRUE); */ /* 儘乕僇儖價儏乕傾桳岠壔乮枹巊梡乯 */
	glEnable(GL_LIGHTING); glEnable(GL_LIGHT0);  /* 儔僀僥傿儞僌桳岠壔 */
	glEnable(GL_NORMALIZE);   /* 朄慄儀僋僩儖惓婯壔 */
	glEnable(GL_DEPTH_TEST);  /* 僨僾僗僥僗僩桳岠壔 */
}
...
{
	static GLfloat v[4]={1.0,-1.0,2.0,0.0}; /* 各富の羹き */
	auxInitPosition(0,0,800,800);
	...
	glLightfv(GL_LIGHT0,GL_POSITION,v);      /* 士乖各俐 */
	glClearColor(0.0,0.0,0.0,0.0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
